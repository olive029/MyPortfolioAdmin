...
const Dashboard = () => {
  ...
  const toggleMenu = (enquiryId, event) => {
    event.stopPropagation();
    if (activeMenu === enquiryId) {
      setActiveMenu(null);
    } else {
      setActiveMenu(enquiryId);
  ...
  const handleRead = async (enquiry) => {
    try {
      const enquiryRef = doc(db, 'contactMessages', enquiry.id);
      ...
      setEnquiries(prev => prev.map(item => 
        item.id === enquiry.id 
          ? { ...item, read: true, readAt: new Date() }
          : item
      ));
      
      console.log('Enquiry marked as read:', enquiry.id);
      closeMenu();
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingEnquiry) {
        ...
        const enquiryRef = doc(db, 'contactMessages', editingEnquiry.id);
        await updateDoc(enquiryRef, {
          name: formData.name,
        ...
        console.log('Enquiry updated successfully:', editingEnquiry.id);
      } else {
        ...
        const docRef = await addDoc(collection(db, 'contactMessages'), newEnquiry);
  ...
  useEffect(() => {
    const fetchEnquiries = async () => {
      try {
        ...
        console.log('✅ Firestore returned:', enquirySnapshot.docs.length, 'documents');
        ...
        console.log('📊 Final enquiries list:', enquiriesList);
  ...
  // Format date - handle both string timestamps and Firestore timestamps
  const formatDate = (timestamp) => {
    ...
    try {
      ...
      if (timestamp.toDate) {
        ...
        // ISO string format (like your existing data)
  ...
  // Format time - handle both string timestamps and Firestore timestamps
  ...
  const [menuItemHover, setMenuItemHover] = useState(null);
        ...
        >
          <button
            style={{
              ...styles.menuItem,
              ...styles.editItem,
              ...(menuItemHover === 'edit' ? styles.menuItemHover : {})
            }}
            onMouseEnter={() => setMenuItemHover('edit')}
            onMouseLeave={() => setMenuItemHover(null)}
            onClick={() => {
              const enquiry = enquiries.find(e => e.id === activeMenu);
              if (enquiry) handleEdit(enquiry);
            }}
          >
            <span>✏️</span>
            Edit
          ...
          <button
            style={{
              ...styles.menuItem,
              ...styles.readItem,
              ...(menuItemHover === 'read' ? styles.menuItemHover : {})
            }}
            onMouseEnter={() => setMenuItemHover('read')}
            onMouseLeave={() => setMenuItemHover(null)}
            onClick={() => {
              const enquiry = enquiries.find(e => e.id === activeMenu);
              if (enquiry) handleRead(enquiry);
            }}
          >
            <span>👁️</span>
            Mark as Read
          ...
          <button
            style={{
              ...styles.menuItem,
              ...styles.deleteItem,
              ...(menuItemHover === 'delete' ? styles.menuItemHover : {})
            }}
            onMouseEnter={() => setMenuItemHover('delete')}
            onMouseLeave={() => setMenuItemHover(null)}
            onClick={() => {
              const enquiry = enquiries.find(e => e.id === activeMenu);
              if (enquiry) handleDelete(enquiry);
            }}
          >
            <span>🗑️</span>
            Delete
          ...
          <div style={styles.modalContainer} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={styles.modalFooter}>
              ...
              <button
                ...
                disabled={submitting}
              >
                {submitting ? 'SUBMITTING...' : (editingEnquiry ? 'UPDATE' : 'SUBMIT')}
              </button>
            </div>
